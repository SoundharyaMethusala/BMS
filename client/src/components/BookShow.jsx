    ...
    const getSeats = ()=>{
        ...
        return(
            ...
            style={{
                ...
                <div className="d-flex bottom-card justify-content-between w-100 
                max-width-600 mx-auto mb-25px mt-3">
    ...
    const onToken = async (token)=>{
        try{
            ...
            if(response.success){
                ...
                console.log(params.id,transactionId,selectedSeats,user._id);
        ...
        message.error("Failed to book the show");       
        }      
    }
            else{
                message.error(response.message);
            ...
            {show && (
                <Row gutter={24}>
                    <Col span={24}>
                        ...
                        </div>} extra={<div className="show-name py-3">
                            <h3><span>Show Name:</span>{show.name}</h3>
                            <h3><span>Date & Time:</span>
                                {moment(show.date).format("MMM Do YYYY")} at{" "}
                                {moment(show.time,"HH:mm").format("hh:mm A")}
                            </h3>
                            <h3><span>Ticket Price:</span>Rs. {show.ticketPrice}/-</h3>
                            <h3><span>Total Seats:</span>{show.totalSeats}
                                ...
                                <StripeCheckout token={onToken} billingAddress
                                amount={selectedSeats.length*show.ticketPrice*100}
                                stripeKey="pk_test_51Q5KVeKcYFgOfsFYJgeM9Btp7oEWrkCIhPLoQs9yhBGsYJRJq2GKfkk2q8FzcVyMXAntDXCioUesyK50j2Wt3lsr006wdHGC4Y"
                                >
                                    {
